...
import { createSqliteDatabase, createSqliteRepositories, importLegacyState } from '../src/index.js';
import { createPendingRequestRecord, createSessionRecord, createThreadPreferenceRecord, createUploadRecord } from '@codex-remote/domain';
...
test('sqlite migrations create repositories that can round-trip records', () => {
  ...
  repositories.sessions.upsertSession(createSessionRecord({
    threadId: 'thread-1',
    name: 'Demo',
    cwd: 'C:\\workspace',
    status: 'idle',
  }));
  repositories.pendingRequests.upsertPendingRequest(createPendingRequestRecord({
    requestId: 'req-1',
    threadId: 'thread-1',
    turnId: null,
    itemId: null,
  ...
  repositories.threadPreferences.upsertThreadPreference(createThreadPreferenceRecord({
    threadId: 'thread-1',
    approvalPolicy: 'on-request',
    sandboxMode: 'danger-full-access',
  }));
  repositories.uploads.upsertUpload(createUploadRecord({
    id: 'upload-1',
  ...
  assert.equal(repositories.threadPreferences.getThreadPreference('thread-1')?.sandboxMode, 'danger-full-access');
...
test('legacy import populates thread preferences and window bindings', () => {
  ...
  const repositories = createSqliteRepositories(database);
  assert.equal(repositories.threadPreferences.getThreadPreference('thread-2')?.approvalPolicy, 'never');
  assert.equal(repositories.windowBindings.listWindowBindings()[0]?.pid, 4321);
  assert.equal(repositories.appState.getAppState('lastWorkspacePath')?.valueJson, JSON.stringify('C:\\workspace'));
  database.close();
});
